...
class TestFileNamer(unittest.TestCase):
	def setUp(self):
		self.test_path = normal_image_path
	...
	def test_generated_paths(self):
		...
		expected_roi_path = os.path.join(normal_experiment_dir, "cellpose_rois/V52_patch_1_fibers_RoiSet.zip")
		self.assertEqual(self.namer.get_path("fiber_rois"), expected_roi_path)

class TestFileNamerBad(unittest.TestCase):
	def setUp(self):
		self.test_path = os.path.join(normal_experiment_dir, "BAD_PATH/non-image.png")
		self.namer = FileNamer(self.test_path)
